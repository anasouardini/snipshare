...
const read = async (req, res) => {
    ...
    if (!response || !response?.[0]) {
        return res.status(500).json({msg: 'something bad happended while feching notifications'});
    }
...
const markAllRead = async (req, res) => {
    ...
    if (!response || !response?.[0]) {
        return res
            .status(500)
            .json({msg: 'something bad happended while marking notifications as read'});
    }
