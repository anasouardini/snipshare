...
const edit = async (req, res) => {
  ...
  const owner = req.params.user;
  const response = await Snippet.editSnippet(
    owner,
    req.body.props,
  ...
  return response[0]?.affectedRows
    ...
    : res.json({
        status: 500,
        msg: `something happend while editting the snippet`,
      });
};
...
const remove = async (req, res) => {
  ...
  return res
    .status(500)
    .json({ msg: `something happend while deleting the snippet` });
};
...
const appendSnippetToResponse = async (
  req,
  filteredSnippets,
  snippetObj,
  access,
...
const readUserAll = async (req, res) => {
  ...
  res.json({
    msg: {
      nextPage:
        filteredSnippets.length / perPage <= pageParam
          ? undefined
          : pageParam + 1,
      snippets: filteredSnippets.slice(
        (pageParam - 1) * perPage,
        pageParam * perPage,
      ),
      genericAccess,
  ...
  if (authorized) {
    ...
    if (response && response[0]?.affectedRows) {
      ...
      if (authorized == 2) {
        ...
        const updateCoworkerRuleResult = await CoworkerRules.update(
          owner,
          rules,
        );
        if (updateCoworkerRuleResult?.[0].length) {
